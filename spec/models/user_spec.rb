...
describe User do
  ...
  describe '#feed_photos_during' do
    it 'should find strong photos taken within the time range' do
      ...
      expect(CameraEvaluator).to receive(:new).with(camera: c1, frame: anything)
        .and_return(c1_eval)
      expect(c1_eval).to receive(:strength).with(kind_of(UserEvaluator))
        .and_return(Camera::MIN_STRENGTH / 2.0)
      expect(CameraEvaluator).to receive(:new).with(camera: c2, frame: anything)
        .and_return(c2_eval)
      ...
      expect(c2.photos).to receive(:during).with(t(0), t(1))
        .and_return([1, 2, 3])
      expect(UserEvaluator).to receive(:new).with(user: subject, frame: anything)
